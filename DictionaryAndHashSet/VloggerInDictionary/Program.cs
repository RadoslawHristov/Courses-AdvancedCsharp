...
namespace _07._The_V_Logger_Exc
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            database = GetAddVlogger(database, input);

            PrintStatisticOfVlgger(database);

        }

        private static Dictionary<string, Dictionary<string, HashSet<string>>> GetAddVlogger(Dictionary<string, Dictionary<string, HashSet<string>>> database, string input)
        {
        ...
        private static void PrintStatisticOfVlgger(Dictionary<string, Dictionary<string, HashSet<string>>> database)
        {
            Console.WriteLine($"The V-Logger has a total of {database.Count} vloggers in its logs.");

            int number = 1;

            foreach (var vlogger in database.OrderByDescending(v => v.Value["followers"].Count)
